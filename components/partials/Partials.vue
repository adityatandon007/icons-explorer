<template>
  <div>
    <section>
      ...
      <SubHeaderContainer v-else :count="count" :query="search" :asset-type="assetTypesMap[filters.asset.value]" description="Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation" />
    </section>
    <main>
        ...
        <section class="px-5 results">
          <div class="scrollable-tags mb-4">
            <div v-if="$fetchState.pending" class="items-grid">
              <b-skeleton v-for="(i, key) in 10" :key="key" height="58px" width="112px" />
            </div>
            <HorizontalScrollableList v-else :itemList="items">
              <template v-slot:default="{ item }">
                <b-button variant="default" size="sm" class="font-weight-normal text-truncate" style="width: 125px;">{{ item }}</b-button>
              </template>
          ...
          <div class="container-fluid px-0 pb-0 bg-white position-relative">
            <div class="mb-6 mt-3">
              ...
              <div v-else class="items-grid">
                <AssetCard v-for="(item, key) in results" :item="item" :key="key" />
              </div>
...
<script>
import SubHeaderContainer from '../containers/SubHeaderContainer.vue'
import HorizontalScrollableList from '../scrollables/HorizontalScrollableList.vue'
import CollapsableItem from '../collapsables/CollapsableItem.vue'
...
export default {
  ...
  async fetch () {
    ...
    const asset = this.queryAssetMap[this.filters.asset.value]
    try {
      this.$axios.setHeader('Client-ID', '13060331799489')
      let items = {}
      if (asset === 'all') {
        const assets = ['lottie', '3d', 'illustration', 'icon']
        const allAssets = assets.map(type => this.$axios.$get(`/search?query=${this.search}&product_type=item&asset=${type}&per_page=5&page=${this.page}&sort=${this.filters.sort.value}`))
        const [{ response: _lottieRes }, { response: _3dRes}, { response: _illusRes}, { response: _iconRes }] = await Promise.all(allAssets)
        items = {
          data: [..._lottieRes.items.data, ..._3dRes.items.data, ..._illusRes.items.data, ..._iconRes.items.data], 
          last_page: Math.min(_lottieRes.items.last_page, _3dRes.items.last_page, _illusRes.items.last_page, _iconRes.items.last_page),
          total: _lottieRes.items.total + _3dRes.items.total + _illusRes.items.total + _iconRes.items.total
        }
      } else {
        const { response } = await this.$axios.$get(`/search?query=${this.search}&product_type=item&asset=${asset}&per_page=20&page=${this.page}&sort=${this.filters.sort.value}`)
        items = response.items
      }
      const { data, last_page, total } = items
      this.results = this.page > 1 ? [...this.results, ...data] : [...data]
      this.items = this.results.slice(0, 10).map((item) => item.name)
      this.lastPage = last_page
      this.count = total
      this.freeAccessExceeded = this.page > 2 
    } catch (error) {
      this.error = `Failed to load ${this.search} ${this.assetTypesMap[assetType]}`
    }
  },
  methods: {
    update (value, key) {
